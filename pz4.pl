...
pr9:-see('C.txt'),read_str_str(A),read_str_str_1(A,Word,Max),write_str(Word),write(Max),seen.

read_str_str_1([H|T],Word,Max):-common_word([H|T],H,Word,[],[],0,Max,H).

repeat_word([],[],[],_,K1,K,1):-K is K1+1,!.
repeat_word([],[],_,_,K,K,_):-!.
repeat_word([_|T],[],[],Sub,K1,K,1):-K2 is K1+1,first_elem(T,EL),repeat_word(T,EL,Sub,Sub,K2,K,0),!.
repeat_word([_|T],[],_,Sub,K1,K,_):-first_elem(T,EL),repeat_word(T,EL,Sub,Sub,K1,K,0),!.
...
common_word([_|T],[],B,ML,[],Max,LM,A):-first_elem(T,EL),common_word(T,EL,B,ML,[],Max,LM,A),!.
