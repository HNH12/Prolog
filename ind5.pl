...
comb:-read(A),write("Введите длину: "),read(N),
		(N>=8,N=<10->
		      comb(A,N,B),(not(sochet(B)),repeat_twice(B,B)->
				  asserta(sochet(B)),list_without_twice_char(B,C),
				   append(B,C,D),write(D),nl,fail);write("Неверная длина"),fail).
comb([],0,[]).
